 ...
 *
 * $Id: layer3.c,v 1.3 2008/08/20 17:56:34 sulei Exp $
 */
/*

    XMOS: optimised the 32KB rq_table out using XMOS proprietary shifting_3rd_root algorithm.
 ...
 */
/*XMOS: No need anymore, optimised out using XMOS proprietary shifting_3rd_root algorithm which is exact using -DEXACT and even more accurate using -DACCURATE.*/
/*static
struct fixedfloat {
  unsigned long mantissa  : 27;
  unsigned short exponent :  5;
} const rq_table0[RQ_TABLE_CORE0] = {
# include "rq_table0.dat"
};
*/
struct fixedfloat {
  unsigned long mantissa  : 27;
 ...
 */
static
enum mad_error III_huffdecode(struct mad_bitptr *ptr, mad_fixed_t *xr,
                              struct channel *channel,
...
{
  short modes[39];
  unsigned int sfbi=0, l=0, n=0, i=0;
...
enum mad_error III_decode(struct mad_bitptr *ptr, struct mad_frame *frame,
                          struct sideinfo *si, unsigned int nch, mad_fixed_t xr[][576])
{
...
unsigned char global_main_data[MAD_BUFFER_MDLEN] = {0};
mad_fixed_t global_overlap[2][32][18] = {{{0}}};
#endif
 ...
 */
int mad_layer_III(struct mad_stream *stream, struct mad_frame *frame,  mad_fixed_t scratch[][576])
{
  struct mad_header *header = &frame->header;
...
#else
    stream->main_data = (&global_main_data);
#endif
    if (stream->main_data == 0) {
  ...
  if (result == 0) {
    error = III_decode(&ptr, frame, &si, nch, (void*)scratch);
    if (error) {
      stream->error = error;
      result = -1;
    }
