 ...
 *
 * $Id: synth.c,v 1.2 2008/08/20 17:56:34 sulei Exp $
 */
                ...
                unsigned int nch, unsigned int ns)
{
  ...
  signed short *scaled_pcm1, *scaled_pcm2;
#else
  ...
  for (ch = 0; ch < nch; ++ch) {
    ...
    phase    = synth->phase;
#ifdef SAMPLES_16_BITS
    scaled_pcm1 = synth->pcm.scaled_samples[ch];
#else
...
#ifdef SAMPLES_16_BITS
      *scaled_pcm1++ = scale(SHIFT(MLZ(hi, lo)));
      scaled_pcm2 = scaled_pcm1 + 30;
#else
...
#ifdef SAMPLES_16_BITS
  *scaled_pcm2-- = scale(SHIFT(MLZ(hi, lo)));
#else
