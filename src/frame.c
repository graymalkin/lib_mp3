 ...
 *
 * $Id: frame.c,v 1.2 2008/08/20 17:56:34 sulei Exp $
 */
...
#ifdef DEBUG_FRAME
#define fprintstr(s) myprintstr(s)
#define fprinthex(x) myprinthex(x)
#else
#define fprintstr(s) {;}
#define fprinthex(x) {;}
#endif
...
int mad_frame_decode(struct mad_frame *frame, struct mad_stream *stream, mad_fixed_t xr[][576])
{
  ...
  switch(frame->header.layer){
    //need to support layer 1 and 2 only for backward ISO compatibility
    case 1:
      goto fail;
//      result = mad_layer_I(stream, frame);
      break;
    case 2:
      goto fail;
//      result = mad_layer_II(stream, frame); 
      break;
    case 3:
      fprintstr("frame: mad_layer_III\n");
      result = mad_layer_III(stream, frame, xr);
      break; 
    default:
      goto fail;
      break;
